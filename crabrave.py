...
def inlinequery(update, context):
    ...
    results = [
        ...
        InlineQueryResultVideo(
            id=uuid4(),
            title="🦀🦀 BOTH RONNIE FERRARI AND QUALITY ARE GONE 🦀🦀",
            description=query,
            thumb_url=f"{BASE_URL}/thumb/onabytakchciala.jpg?t={time.time()}",
            video_url=f"{BASE_URL}/video/{parse.quote_plus(query)}.mp4?style=onabytakchciala_poor&font=comicsans&color=white&size=52&filter=snapchat",
            mime_type="video/mp4"
        ),
        InlineQueryResultArticle(
            id=uuid4(),
